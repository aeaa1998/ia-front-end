<template>
    <div class="uk-container uk-container-large">
        ...
        <ul class="uk-switcher uk-margin">
            ...
            <li class="uk-margin-small uk-flex">
                <video ref="videRefBowl" @timeupdate="() => playing(30, 'bowl')" class="uk-height-max-medium uk-margin-right" src="../assets/bowl_1.mp4"  autoplay="false" controls playsinline uk-video></video>
                <div class="uk-card uk-border-rounded uk-card-default uk-width-1-1 uk-height-max-medium uk-overflow-auto">
                    <div class="uk-padding-small uk-text-left" style="max-height: 100%">
                        <div class="uk-card-title ">Datos relevantes</div>
                        <div class="">
                            <div class="uk-text-bold">Bola de boliche</div>
                            <div class="uk-grid uk-grid-small">
                                <div class="uk-width-1-2" v-for="i in bowlData(frames.bowl)" :key="i.title">
                                    <div class="uk-flex uk-flex-column">
                                        <div class="uk-text-small">{{i.title}}</div>
                                        <div class="uk-text-meta">{{i.value}}</div>
                                    </div>
...
export default {
    ...
    beforeMount(){
        ...
        this.results.bowling = Object.keys(bowlTrack1).reduce((carry, key) => {
    ...
    methods:{
        playing(frameRate, who='ping'){
            if(who == 'ping'){
                this.frames.pingPong = Math.floor(this.$refs.videRef.currentTime * frameRate)
            }else{
                this.frames.bowl = Math.floor(this.$refs.videRefBowl.currentTime * frameRate)
            }
        ...
        bowlData(_frame){
            ...
            let movement;
            try {
                movement = this.results.bowling['Ball'][`${frame}`]['movement']
            } catch {
            ...
            const x = {
                title:  'Movimiento (cm)', 
                value: (movement['x']/2.4).toFixed(2)
            }
            
            const x_speed = {
                title:  'Vel (cm/s)', 
                value: (movement['x_speed']/2.4).toFixed(2)
            }

            const x_speed_avg = {
                title:  'Vel promedio (cm/s)', 
                value: (movement['x_speed_avg']/2.4).toFixed(2)
            }
            return [
                x,x_speed,x_speed_avg
            ]
        },
        bollData(who, _frame) {
            if(_frame == 0){
